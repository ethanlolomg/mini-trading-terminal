/**
 * Raydium CLMM swap math — single-step swap computation + swap state.
 *
 * Adapted from Raydium SDK V2 (src/raydium/clmm/libraries/swapMath.ts),
 * Apache-2.0. https://github.com/raydium-io/raydium-sdk-V2
 *
 * Edits vs upstream: `@/common` (FEE_RATE_DENOMINATOR_VALUE) -> ./constants;
 * layout `ReturnType<...>` types -> ./types; `./pool` -> ./poolFee;
 * `./tickArrayUtil` -> ./tickMath; `./bigNum` -> ./fixedPoint.
 */
